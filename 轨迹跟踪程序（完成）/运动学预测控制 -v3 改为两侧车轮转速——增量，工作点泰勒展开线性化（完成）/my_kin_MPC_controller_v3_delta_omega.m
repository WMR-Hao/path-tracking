...
function [u_out]=my_kin_MPC_controller_v3_delta_omega(body_pos,body_vel,ref_position,dt,Np,Nc)
...
% 此处：delta_X=AX+BU ————X=[a-r]=[X_I-Xr;Y_I-Yr;ALPHA_I-ALPHAr]statu_e=A*statu_e+B*u_e   %离散后的模型
    % OUT=delta_X=a-r
A=[ 0, 0, -wheel_r*((omega_l+omega_r)/2)*sin(PSI) ;  
    0, 0,  wheel_r*((omega_l+omega_r)/2)*cos(PSI) ;
    0, 0,  0]; 
...
% A=[ 0, 0, -wheel_r*((omega_l+omega_r)/2)*cos(PSI) - vy*sin(PSI) ;  % 有问题        
%     0, 0,  wheel_r*((omega_l+omega_r)/2)*sin(PSI) + vy*cos(PSI);
%     0, 0,  0]; 
% B = [wheel_r*cos(PSI)/2    wheel_r*cos(PSI)/2;
%      wheel_r*sin(PSI)/2    wheel_r*sin(PSI)/2;
%           -wheel_r/(2*d)     wheel_r/(2*d);];
...
for prediction_time=1:Np
    ...
    omega_l=ut(1);  % k|t时刻的输入量 标称轨迹上，输入恒定的输入量
    omega_r=ut(2);
    
    vxt = wheel_r * ( omega_l+omega_r )/2 ; 
    gammat = wheel_r * ( -omega_l + omega_r )/( 2*d ) ;
