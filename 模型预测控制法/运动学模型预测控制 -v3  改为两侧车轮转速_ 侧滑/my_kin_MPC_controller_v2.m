...
function [u_out]=my_kin_MPC_controller_v2(body_pos,body_vel,ref_position,vxr,vyr,gammar,dt,beta)
%  预测时域Np	控制时域Nc
Nc = 10  ; 
Np = 40  ;
...
% 此处：delta_X=AX+BU ————X=[a-r]=[X_I-Xr;Y_I-Yr;ALPHA_I-ALPHAr]statu_e=A*statu_e+B*u_e   %离散后的模型
    % OUT=delta_X=a-r
A=[ 1, 0, dt*(- vxr*sin(PSIr) - vxr*cos(PSIr)*tan(beta));
    0, 1,  dt*( vxr*cos(PSIr) - vxr*sin(PSIr)*tan(beta));
    0, 0,   1]; 
B = [dt*(cos(PSIr) - sin(PSIr)*tan(beta)),  0;
     dt*(sin(PSIr) + cos(PSIr)*tan(beta)),  0;
     0,                                  dt;];
