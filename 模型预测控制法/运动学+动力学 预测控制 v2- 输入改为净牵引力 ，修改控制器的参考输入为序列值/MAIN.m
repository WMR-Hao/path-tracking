...
%  Faa=1*sin(t);
% Taa=(0.5*ones(size(t,2),1))';% 6.4746
...
for i=1:length(t)
    ...
    %tic
    %     %************************   控制器部分  *************************
    u_out = my_dyn_MPC_controller_v9_backup...
    ...
    %% 执行器赋值    模拟机器人运动
    [X,Y,PSI,vx,vy,gamma] = dynamics_model_v9_backup(Fa,Fb,X,Y,PSI,vx,vy,gamma,dt,alpha);
